...
> 本文形成于核心模块完成 high-level 讨论之前，仅保留为候选机制素材。
> 当前架构入口见 [`../../ARCHITECTURE.md`](../../ARCHITECTURE.md)。
...
├── mission.json      # schema 版本、Mission ID、状态、风险与创建时间
...
价格来自独立、版本化的模型事实目录，不写入 Armor 质量分。API 现金与订阅额度分别记账；每个 Mission 冻结当时的 price card，后续价格变化不回算历史。过期或无法从官方来源复核的价格只能标为未知，不能被当作零成本。
