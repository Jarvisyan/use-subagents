...
> Maturity：Superseded
> Original maturity：Proposed
> 本文中的 P0–P6、模型编制和退出条件尚未完成逐模块论证，不是当前执行计划。
> 当前计划入口见 [`../../PLAN.md`](../../PLAN.md)。
...
2. P1 的 JARVIS、MARK-P、MARK-X 和 MARK-R 全部使用 GPT-5.6 Sol；各角色使用独立上下文，契约、产物和绩效分开记录。
3. Plan、Execute、Check 职责固定，Agent 数量根据风险和可并行性弹性变化。
4. 共享工作区只有一个 Writer；Reviewer 使用全新上下文独立审查。
5. 首个 Demo 只使用 GPT 旗舰模型，避免流程、供应商和模型档位同时变化。
6. 后续对照只把 MARK-X：Executor 从 GPT-5.6 Sol 替换为 DeepSeek V4 Pro。
7. Kimi K3、GLM-5.2 等其他旗舰模型不常驻；仅在 Veronica 触发条件成立时择一作为异构 MARK-P：Planner Challenger。
...
12. 当前正式路线不使用 GPT 次旗舰、DeepSeek V4 Flash 或其他次旗舰模型；以后如要引入，必须作为单独 Candidate 通过质量门槛，而不是只凭低价上岗。
13. 模型 API 价格与限制将维护为带官方来源、核验时间和历史版本的事实目录；它只服务成本计算，不参与质量评分。
...
- [x] 初始化 Git 仓库并保留首个存档 commit。
- [ ] P1 开跑前提交已批准的设计文档，记录基线 commit hash 与回滚命令。
...
- [ ] 用文档和人工编排跑通 P1；项目本地 Skill 延后到连续试点证明协议稳定之后，避免过早固化。
...
- 校验必需文件、最小 manifest 字段、当前状态枚举和风险枚举；
- 输出缺失项和可定位的失败原因；
- 提供自动化测试和可复现命令。

P1 只实现 `mission init` 与 `mission check`：`init` 只写入初始状态，不提供状态迁移命令；CLI 不执行 Agent、不调用模型，也不判断证据的语义质量。状态迁移、完整验收—证据映射和自动路由在真实使用暴露需求后再进入后续阶段。

它不是纯玩具：这个工具会成为后续每次 Mission 的 Arc Reactor 一部分；同时它范围适中、可回滚、有强客观测试，适合验证组织流程。详细设计见 [`DEMO_DESIGN.md`](DEMO_DESIGN.md)。
...
| MARK-X：Executor / Writer | 独立上下文中的 GPT-5.6 Sol，High，唯一 Writer |
| MARK-R：Reviewer | 无历史独立任务中的 GPT-5.6 Sol，High |
| MARK-S：Scout / Researcher | 默认不启动；触发时使用只读 GPT-5.6 Sol |
| Arc Reactor：Mechanical Verifier | 单元测试、静态检查和命令行验收 |

JARVIS 是 Chair，不兼任默认 Writer 或 Reviewer。P1 使用独立 MARK-P 上下文来验证委派链；MARK-P、MARK-X、MARK-R 都是一个起步人数，而不是永久上限。只有依赖图出现可独立、边界明确且不会争抢写入权的工作流时，才启动 House Party；第一轮不为展示并行而并行。
...
- [ ] 覆盖 Green、Amber，并做至少一次受控 Red 升级演练：验证流程会停止并请求裁决，不在 P2 交付真实 Red 实现。
...
- [ ] 建立 `data/model_catalog.json` 与说明文档，维护官方模型 ID、能力限制、原生币种价格、订阅额度、来源、核验时间和 TTL。
- [ ] 价格变化新增 price card 并保留历史，不覆盖旧 Mission 的成本依据；README 简表由目录生成，不手工维护第二份。
- [ ] 定时 CI 只负责标记即将过期、已过期或来源变化的记录；只有重新核对官方字段后才能刷新核验时间或合并新 price card。
- [ ] API 现金成本与 Codex 订阅额度分开记账；价格事实与 Armor 质量分严格分离。

#### 实验规则

1. 从同一基线 commit 建立两个隔离 worktree。
2. A 组使用 GPT-5.6 Sol High 的 MARK-X 档案，B 组使用 DeepSeek V4 Pro 的 MARK-X Candidate。
...
9. DeepSeek V4 Flash 和其他次旗舰不进入本轮 A/B；若未来评估，另开同样受控的 Candidate 实验。
10. 实验启动前锁定当时的官方价格卡、模型 ID、上下文限制与核验时间，历史 Mission 不随新价格回算。
...
### P4 — Veronica Experiment：测试异构旗舰 MARK-P Challenger 档案

**目标：** 判断异构规划何时真正发现新问题，而不是把互博变成常驻仪式。

#### 协议

1. Sol MARK-P 与一个异构旗舰 MARK-P Challenger 从同一份冻结 Brief 独立盲规划；候选可为 Kimi K3 或 GLM-5.2，但一次实验只选一个。
...
- Tony Stark 审批 [`DEMO_DESIGN.md`](DEMO_DESIGN.md) 中的 D1–D5。
- Tony Stark 审批 Mission 001 的 Brief、Acceptance、非目标和暂定风险。
- JARVIS 按 `DEMO_DESIGN.md` 的 Preflight Checklist 确认 Reviewer 隔离、首轮输入白名单和证据保存方式。

### P3 前解决

- A/B 的具体任务集合和统计门槛。
- DeepSeek 可访问的仓库范围、许可证与隐私条件。
- DeepSeek V4 Pro 的官方模型 ID、价格卡、上下文限制和数据保留条款。
...
1. 完成薄 P0：确认基线与回滚、Mission 001 暂定风险、Reviewer 白名单和证据保存方式；完整风险规范与本地 Skill 不阻塞 P1；
2. 由 Tony Stark 通过 Gate A，审批 P1 Demo 的 D1–D5；
3. 冻结 Mission 001 的 Brief、Acceptance、非目标和暂定风险，并由 Tony Stark 通过 Gate B；
4. 两道 Gate 都通过后，启动全 GPT-5.6 Sol 的 P1 Demo；
5. 复盘至少三个 Mission 后，再进入 DeepSeek V4 Pro A/B；
6. 只有 Veronica 条件真实触发时，才择一启动 Kimi 或 GLM 规划实验；
7. P1–P4 数据充分后先用 Routing Shadow 校准动态权重；
8. 通过干净 clone 验证后，再发布 GitHub `v0.1.0`。
